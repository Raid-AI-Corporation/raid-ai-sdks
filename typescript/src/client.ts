...
export interface RaidClientOptions {
  ...
  apiKey: string;
  /** The Raid AI API base URL (required) — e.g. from your own config or `process.env.RAID_API_BASE_URL`. */
  baseUrl: string;
 ...
 *   baseUrl: process.env.RAID_API_BASE_URL!,
 * });
...
export class RaidClient {
  ...
  constructor(options: RaidClientOptions) {
    ...
    if (!options.baseUrl) {
      throw new Error("RaidClient: `baseUrl` is required (the Raid AI API base URL).");
    }
