...
export interface DocumentAnalyzeOptions {
  ...
  /**
   * Whether to also extract and check the document's data when `docType` is one of the
   * extractable types (`commercial_registry`, `tax_card`, `utility_bill`). Defaults to on
   * server-side; pass `false` to skip extraction for this upload. Ignored for every other
   * type — extraction is decided from the submitted `docType`, never from the classifier.
   */
  extractData?: boolean;
}
...
export class Documents {
  ...
  analyze(file: FileInput, options: DocumentAnalyzeOptions = {}): Promise<DocumentAnalysis> {
    ...
    if (options.extractData !== undefined) form.append("extractData", String(options.extractData));
   ...
   *
   * Terminal means the verdict is final. Data extraction (the three extractable
   * `docType`s) finishes after the verdict, so `dataStatus` may still be `pending` on the
   * returned record — keep polling {@link get} if you need the extracted data.
   */
