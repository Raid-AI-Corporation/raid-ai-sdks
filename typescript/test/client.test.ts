...
describe("RaidClient construction", () => {
  ...
  it("defaults the base URL to production when omitted", async () => {
    const { fetch, calls } = jsonFetch(200, {});
    const raid = new RaidClient({ apiKey: "test-key", fetch, maxRetries: 0 });
    await raid.video.getJob("abc");
    expect(calls[0]?.url).toBe("https://api.raidxai.com/api/app/video-forensics/jobs/abc");
  });
...
describe("document analysis", () => {
  ...
  it("sends extractData only when given, as a form string", async () => {
    const { fetch, calls } = jsonFetch(202, analysis("processing"));
    const raid = makeClient(fetch);
    const file = { data: new Uint8Array([1]), fileName: "card.pdf", contentType: "application/pdf" };

    await raid.documents.analyze(file, { docType: "tax_card" });
    await raid.documents.analyze(file, { docType: "tax_card", extractData: false });

    expect((calls[0]?.init?.body as FormData).get("extractData")).toBeNull();
    expect((calls[1]?.init?.body as FormData).get("extractData")).toBe("false");
  });

  it("surfaces the extraction leg on the record", async () => {
    ...
    const res = await makeClient(fetch).documents.get("doc-1");
    expect(res.dataStatus).toBe("in_review");
    expect(res.extraction?.fields?.tax_id).toBe("300123456");
    expect(res.extraction?.fieldChecks?.tax_id?.status).toBe("ok");
  });
...
describe("credits and limits", () => {
  it("reads each modality's own credit-info route", async () => {
    ...
    const audio = await raid.audio.creditInfo();
    expect(audio.tenantCredits).toBe(480);
    // Audio accepts any decodable file; the flag is what new clients should read, the
    // deprecated extension list keeps deserializing for old ones.
    expect(audio.acceptsAnyAudio).toBe(true);
    expect(typeof audio.allowedFileFormats).toBe("string");
