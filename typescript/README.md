...
const raid = new RaidClient({
  ...
  // baseUrl: "https://api.raidxai.com",      // optional — defaults to the production API
  // timeoutMs: 60_000,
  // maxRetries: 2,
...
content, not its name or `contentType`, and the audio is normalized server-side before analysis.
...
`tax_card` and `utility_bill`, only when you submit that `docType`. The result is a second status
beside the verdict, never part of it:

```ts
const card = await raid.documents.analyzeAndWait(file, { docType: "tax_card" });
// analyzeAndWait resolves on the VERDICT; extraction lands afterwards.
let record = card;
while (record.dataStatus === "pending") {
  await new Promise((r) => setTimeout(r, 3000));
  record = await raid.documents.get(record.id);
}
if (record.dataStatus === "verified") console.log(record.extraction?.fields?.tax_id);
if (record.dataStatus === "in_review") console.log(record.extraction?.reasons);
```

`dataStatus` is `pending` / `verified` / `in_review` / `failed`, or `null` with
`extractionSkipReason` saying why (`unsupported_type`, `declined_at_upload`, `disabled`). Pass
`{ extractData: false }` to skip extraction for an upload you only want a verdict on.
...
modality with no format list: `allowedFileFormats` is always `"*"` (kept for older clients) and
`acceptsAnyAudio` is `true`.
