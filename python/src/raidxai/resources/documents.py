...
class Documents:
    ...
    def analyze(
        ...
        *,
        ...
        ``extract_data`` controls whether the document's data is also extracted and checked
        when ``doc_type`` is one of the extractable types (``commercial_registry``,
        ``tax_card``, ``utility_bill``). Defaults to on server-side; pass ``False`` to skip
        it for this upload. Ignored for every other type.
        ...
        charge.
        """
        spec = _specs.document_analyze(file, _doc_type(doc_type), extract_data)
    ...
    def analyze_and_wait(
        ...
        *,
        ...
        Terminal means the verdict is final. Data extraction finishes after the verdict, so
        ``data_status`` may still be ``pending`` on the returned record — keep polling
        :meth:`get` if you need the extracted data.
        """
        submitted = self.analyze(file, doc_type=doc_type, extract_data=extract_data)
...
class AsyncDocuments:
    ...
    async def analyze_and_wait(
        ...
        *,
        ...
        submitted = await self.analyze(file, doc_type=doc_type, extract_data=extract_data)
        if _is_done(submitted):
            if on_poll is not None:
                on_poll(submitted)
            return submitted
