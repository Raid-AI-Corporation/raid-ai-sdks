...
with RaidClient(api_key="<your-api-key>") as raid:
...
A commercial registry extract, tax card or utility bill submitted with that `docType` also has its
**data extracted and checked**. That is a second status beside the verdict, never part of it:
`dataStatus` / `data_status` is `pending`, `verified`, `in_review` or `failed`, and the values and
per-field checks are on `extraction`. It lands **after** the verdict — `analyzeAndWait` /
`analyze_and_wait` return on the verdict, so keep polling `get` while `dataStatus` is `pending` if
you need the data. Pass `extractData: false` / `extract_data=False` to skip it for an upload.
...
| `baseUrl` / `base_url` | production API | Override to target another Raid AI API tier. |
